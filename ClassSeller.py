...
# Cria a tabela de Comissôes
    def create_Commission_Table(self) -> None:
        connection = sqlite3.connect("DataBase.db")
        cursor = connection.cursor()
...
# Alem de mostra media da venda de um vendedor ##
    def show_General_Sales(self) -> None:
        # Conectando ao banco de dados
        connection = sqlite3.connect("DataBase.db")
        cursor = connection.cursor()

        # Consulta para obter o volume de vendas
        # e média de vendas por canal e estado
        query_general = """
        SELECT s.State, sa.Sales_channel, sa.Sallers_name,
               SUM(sa.Sale) AS Total_Sales,
               AVG(sa.Sale) AS Average_Sale_Per_Professional
        FROM Sales sa
        ...
        for row in results_general:
            ...
            print(f"{state:<10} {channel:<20} {seller:<20}"
                  f"{total_sales:<20.2f} {avg_sale:<30.2f}")
...
# Adciona ou atualiza os dados de um Vendedor
# quando inserido nos atributos da classe no main
# Alem disso essa função verifica se já existe um cpf cadastrado,
# se sim aplica um UPDATE para atualizar as informalçioes,
# se não aplica um INSERT para adicionar##
    def update_Seller(self) -> None:
        connection = sqlite3.connect("DataBase.db")
        cursor = connection.cursor()
...
# Substitui os dados da tabela Sallers pela planilha Exccel Vendedores.xlsx #
    def replace_Seller_Table(self) -> None:
        connection = sqlite3.connect("DataBase.db")
        cursor = connection.cursor()
...
# Insere os dados da planilha Vendas.xlsx no banco de dados SQLite
    def insert_Sales_Table(self) -> None:
        connection = sqlite3.connect("DataBase.db")
        cursor = connection.cursor()
        ...
        for index, row in table_sales.iterrows():
            ...
            cursor.execute(
                        "INSERT INTO Sales(Date_of_Sale, Sallers_name, Sale,"
                        "Client_type, Sales_channel, Cost_of_sale)"
                        "VALUES (?, ?, ?, ?, ?, ?)",
                        (date_str, row.iloc[1], row.iloc[2],
...
# Deleta o cadastro de um vendedor no banco de dados pelo CPF
    def delete_Seller(self, cpf: str) -> None | bool:
        connection = sqlite3.connect("DataBase.db")
        cursor = connection.cursor()
        cpf_local = cpf
...
# Funcão para calcular as comissões usando a tabela Sales e
# tbm cria a tabela de comissão
...
# retorna None caso as tabelas foram criadas corretamente ou uma mensagem de erro caso não
    def data_Base_its_open(self) -> tuple | None:
                import os
                os.system("cls")
                try:
                    connection = sqlite3.connect("DataBase.db")
